...
class Server:
    ...
    def __init__ (self, ip):
        ...
        self.opc = self.socket.bind("tcp://*:"+str(ip))
        self.id = 0
        self.intResp = [0, 20]
        self.pred = "a"
            ...
            if ipGenesis2 == 0:
                ...
                if (idAnt > self.intResp[0]) and (idAnt < self.intResp[1]):
                    ...
                    if self.pred == "a":
                        ...
                        self.opc = self.socket.connect("tcp://localhost:"+self.pred)
                        self.socket.send_multipart([self.ip.encode('utf-8'), ipAnt.encode('utf-8'), 'pred'.encode('utf-8')])
                        self.socket.recv_multipart()
                        self.socket = self.context.socket(zmq.REP)
                        self.opc = self.socket.bind("tcp://*:"+self.ip)
                        ipGenesis2 = 0
                ...
                self.socket =self.context.socket(zmq.REQ)
                self.opc = self.socket.connect("tcp://localhost:"+ipGenesis2)
                hola = "hola"
